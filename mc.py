...
class MonteCarloSimulator:
    ...
    def run_simulation(self) -> List[TVLMilestoneStats]:
        milestone_data = {goal[0]: [] for goal in self.tvl_goals}
        treasury_at_milestone = {goal[0]: [] for goal in self.tvl_goals}
        nodes_at_milestone = {goal[0]: [] for goal in self.tvl_goals}
        referral_rewards_at_milestone = {goal[0]: [] for goal in self.tvl_goals}
            ...
            for step in range(self.max_steps):
                ...
                for goal in self.tvl_goals:
                    ...
                    if target not in reached_goals and current_tvl >= target:
                        milestone_data[target].append(step)
                        treasury_at_milestone[target].append(network.treasury)
                        nodes_at_milestone[target].append(len(network.get_active_nodes()))
                        referral_rewards_at_milestone[target].append(
        ...
        for goal in self.tvl_goals:
            target = goal[0]
            times = milestone_data[target]
            treasuries = treasury_at_milestone[target]
            nodes = nodes_at_milestone[target]
            rewards = referral_rewards_at_milestone[target]
